...
input_file_name = config[run_name]["input_file_name"]
...
input_data_df = utils.load_simple_input_data(base_dir, release_name, input_file_name, output_project_fields, id_field, location_field, precision_field)
...
for i in osm_grouped_gdf.feature_type.unique():
    ...
    # type_gdf["fid"] = range(1, len(type_gdf)+1)
    type_gdf.drop(columns=["feature_type"], inplace=True)
    # type_gdf_dict[i] = type_gdf
...
utils.export_combined_data(combined_gdf, output_dir, by_finance_type=False)
